...
const ENABLED_LANGUAGES = ['mermaid', 'markdown'];
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const disposable = vscode.commands.registerCommand(`${pkgJson.name}.ShowPreview`, () => {
    ...
    if (!editors.length || !ENABLED_LANGUAGES.includes(activeEditor?.document.languageId ?? '')) {
      // 提示
      vscode.window.showInformationMessage('Please preview a mermaid file!');
      return;
    }
    preview.createOrShow(activeEditor?.document.languageId ?? '', activeEditor?.document.getText() ?? '');
  });
...
class MermaidPreview {
  ...
  createOrShow(languageId: typeof ENABLED_LANGUAGES[number], code: string) {
    if (this.#panel) {
      this.renderByLanguage(languageId, code);
  ...
  renderByLanguage(languageId: typeof ENABLED_LANGUAGES[number], code: string) {
    if (languageId === 'mermaid') {
      this.render(code);
    }
    else if (languageId === 'markdown') {
      const graph = extractMermaidDiagramsModern(code);
      if (graph) {
        this.render(JSON.stringify(graph));
      }
    }
  }

  isEnabledLanguage(languageId: string = '') {
    return ENABLED_LANGUAGES.includes(languageId);
  }
  ...
  createPanel() {
    ...
    vscode.workspace.onDidChangeTextDocument(debounce({ delay: 100 }, (e) => {
      if (!this.isEnabledLanguage(e.document.languageId)) {
        return;
      }
      if (e.document.uri.toString() === vscode.window.activeTextEditor?.document.uri.toString()) {
        // this.render(e.document.getText());
        this.renderByLanguage(e.document.languageId ?? '', e.document.getText());
      }
    }), null, this.#disposables);

    vscode.workspace.onDidChangeConfiguration(debounce({ delay: 100 }, (e) => {
      if (e.affectsConfiguration('mermaid')) {
        if (!this.isEnabledLanguage(vscode.window.activeTextEditor?.document.languageId)) {
          return;
        }
        // this.render(vscode.window.activeTextEditor?.document.getText() ?? '');
        this.renderByLanguage(vscode.window.activeTextEditor?.document.languageId ?? '', vscode.window.activeTextEditor?.document.getText() ?? '');
      }
    }), null, this.#disposables);
